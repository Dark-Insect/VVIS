...
<div class="w3-sidebar w3-black w3-bar-block" style="width:15%">
  ...
  <a href="{{ route('drivers_information') }}"  class="w3-bar-item w3-button w3-grey" class="w3-bar-item w3-button">Driver's Information</a>
        ...
        <div class="topnav">
            ...
            <div class="search-container">
              <form action="">
                ...
                <a href="{{ route('drivers_information')}}" type="button"><i class="reset">Reset Search</i></a>
              </form>
            </div>
          </div>
    </div>
...
<div class="card-body">
    <a href="{{ route('drivers.create') }}" class="btn btn-success btn-sm" title="Add">
        Add New
    </a>
    <a href="{{ route('pdf.driver') }}" class="btn btn-warning btn-sm" title="Export">
        Export PDF
    </a>
                                ...
                                <thead>
                                 ...
                                 @foreach($drivers_information as $item)
                                    <tr>
                                       ...
                                       <td>
                                          <a href="{{ route('drivers.view', $item->id)}}" title="View"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> View</button></a>
                                            <a href="{{ route('drivers.update',$item->id) }}" title="Edit"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button></a>

                                            <form method="POST" action="{{ route('drivers.destroy', $item->id)}}" accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
